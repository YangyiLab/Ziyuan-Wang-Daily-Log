...
基因型和基因比例效果图
![avartar](https://pic1.zhimg.com/80/v2-29414a66c5633037eb84c9acb931132c_720w.jpg)
...
该莫兰模型假定世代重叠。在每个时间步长，选择一个个体进行繁殖，并选择一个个体死亡。因此，在每个时间步中，给定等位基因的拷贝数可以增加1、减少1，也可以保持不变。这意味着转移矩阵是三对角矩阵(连续时间马尔可夫)
...
## TensorFlow配置
配置完成，位于服务器42.193.15.162中
Jupiter文件和示例学习文件位于/home/ubuntu/tensorflowStudy

## 群体遗传-群体结构和基因流
基因流**gene flow**：通过个体或者配子的移动迁徙，一个等位基因从一个群体进入到另一个群体。

群体结构**population structure**：由于基因流有限，造成等位基因频率在群体中分布存在异质性。
地理隔离形成的种群分化称之为隔离分化vicariance。

![隔离分化](https://pic4.zhimg.com/80/v2-1725d8be4053e4f47a464a496f9eabab_720w.jpg)

图中一条河流分割了两个种群，两个种群之间基因flow比较小，大群体没有Hardy-W平衡两个小种群有hw平衡

距离隔离: 距离较为近的个体之间优先交配

基因流识别
亲本识别：一个配子提供者已知，找到另一个亲本
亲本分析可能会有四种结果：

1）鉴定出唯一一个可能的亲本、即基因流方向

2）有多个可能的亲本。采样时由于是一个亲本的多个种子，不同种子遗传信息不同。此时通常选择因为随机因素导致误认为亲本概率最低的那个，同时还需要更多的证据来选择出亲本，比如选择两个亲本是否生殖期重叠等。

3）没有得到任何可能的亲本。可能的原因是另一个未知亲本不在你的研究样本中，比如可能来自一个超远距离传播的配子。这种情况称之为off-plot gene flow。当然，也有可能是未知亲本发生的突变或者测序分型错误导致的。

4）错误的识别出未知亲本。真正的亲本不在样本中，这时基因流是隐秘基因流cryptic gene flow。
